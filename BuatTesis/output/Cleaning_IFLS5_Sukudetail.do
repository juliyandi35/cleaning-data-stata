...
*Gabung dta
mmerge hhid14 using "IFLS5\bk_sc1.dta"
drop if _merge==2
drop _merge
...
gen race=0 
replace race=1 if suku==1
replace race=2 if suku==2
replace race=3 if suku==9
replace race=4 if suku==4
replace race=5 if suku==3
replace race=6 if suku==8
replace race=7 if suku==5
replace race=8 if suku==17
replace race=9 if suku==10

label def rc 1 "Jawa"
label def rc 2 "Sunda", add
label def rc 3 "Minang", add
label def rc 4 "Batak", add
label def rc 5 "Bali", add
label def rc 6 "Sasak", add
label def rc 7 "Bugis", add
label def rc 8 "Betawi", add
label def rc 9 "Banjar", add
label def rc 0 "Lainnya"
label value race rc
...
mmerge pidlink hhid14 using "output\identitas14"
drop _merge
save "output\notschool14", replace
...
mmerge pidlink hhid14 using "output\notschool14"
drop _merge
...
mmerge pidlink hhid14 using "output\perkawinanA14"
drop _merge
...
mmerge pidlink hhid14 using "output\perkawinanB14"
drop _merge
...
label define MIGRASI 1 "pernah migrasi" 0 "tidak pernah migrasi"

mmerge pidlink hhid14 using "output\melahirkan14"
drop _merge
...
mmerge pidlink hhid14 using "output\MIGRASI14"
drop _merge
...
replace HOURS_WEEKLY=0 if (HOURS==.)
...
mmerge pid14 hhid14 using "output\Pekerjaan14"
drop _merge
...
mmerge pidlink hhid14 using "output\PekerjaanLengkap14"
drop _merge
...
mmerge pidlink hhid14 using "output\Job"
drop _merge
